...
import { priceFormat } from '../components/helper/functions';
...
const OrderBox = () => {
    ...
    const orderHandler = async () => {
        const res = fetch("https://aminsheibani.ir/orders", {
            method: "POST",
            body: JSON.stringify({ order }),
            headers: { "Content-Type": "application/json" },
        });
        const setResponse = await (await res).json()
    ...
    return (
        <section className='order-box w-[90%] h-full m-auto flex-vr'>
            <div className='w-full text-white text-center my-9 flex-hz justify-between items-end'>
                ...
                <div className='logo'>
                    <img className='w-20 md:w-[180px] h-20 md:h-[180px]' src={Logo} alt="logo" title='logo' />
                </div>
            </div>
            ...
            <div className='order-sum fixed bottom-16 md:bottom-32 md:rounded-[26px] animate__bounceInUp'>
                <input className='hidden'
                    value={order}
                    onChange={(e) => setOrder(e, target.value)}
                />
                <button className='text-white text-sm font-bold w-64 md:w-[475px] h-12 md:h-[77px] bg-primary-orange rounded-2xl flex-hz justify-evenly items-center'
                    type='submit'
                ...
                >
                    ...
                    <span className='md:text-2xl'>{priceFormat(state.total)} T</span>
                    <span className='md:text-2xl'>نهایی کردن سفارش</span>
